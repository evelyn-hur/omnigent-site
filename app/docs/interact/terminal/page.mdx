...
- **Persistent.** Every session has a conversation ID and lives beyond the current terminal window. Close your laptop, come back tomorrow, resume where you left off: `omni resume <conversation_id>`. The resume picker lists only sessions you own — resuming binds a runner to the session, which the server allows only for its owner, so sessions merely shared with you are not offered.
