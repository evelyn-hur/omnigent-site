...
export const metadata = pageMeta(
  "Configuration",
  "Configure built-in and custom ACP harnesses, authentication, runtime selection, launch overrides, and native diagnostic capture.",
  {
    eyebrow: "Harnesses",
...
## Capture native harness diagnostics

Native harnesses (`codex-native`, `claude-native`) write their own diagnostics to
channels Omnigent does not log by default. Set
`OMNIGENT_HARNESS_STDERR_ENABLED=1` in the environment that launches the host or
CLI to export that text into Omnigent's logs; the host forwards the flag to
runners. It defaults off, and `OMNIGENT_HARNESS_STDERR_ENABLED=0` disables it.
Running processes keep their launch environment, so start a fresh host and
session after changing it.

| Harness | Diagnostic source | When exported |
| --- | --- | --- |
| `codex-native` | App-server stderr pipe | Continuously for the app-server's lifetime, plus a snapshot when the first thread fails to start |
...
Captured text lands in the owning process's ordinary local logs (runner logs
under `~/.omnigent/logs/runner/`) and in structured event attributes, so it also
reaches any debug-log or OpenTelemetry exporter you have configured. The flag
does not enable an exporter or pick a destination.

Known credential patterns are redacted before export, but treat captured
diagnostics as potentially sensitive.

### Codex continuous diagnostics

With the flag enabled, both `omni codex` and host-managed Codex sessions drain
the app-server's stderr for the whole process lifetime and emit it in batches at
`INFO` roughly every 250 ms — you do not have to wait for a startup timeout to
see runtime output. Codex captures only the app-server's stderr; it does not read
the TUI's own stderr or private log files. A quiet stderr pipe produces no
events.

To find these events, filter your local logs or debug-log sink by session ID and:

- `event_name = 'harness_diagnostic_output'`
...
| `offset` | Raw stderr bytes consumed so far, including omitted bytes |
| `text` | Recent redacted records, at most 65,536 UTF-8 bytes per event |
| `truncated` | Whether any records or bytes were omitted |
| `lines_omitted`, `bytes_omitted` | Records dropped by the 1 MiB / 256-record handoff queue or the export buffer |
| `tail_byte_limit` | Maximum exported text size (65,536 bytes) |

Each event carries the session ID current when the batch is emitted, so output
buffered just before a session switch such as `/clear` can be attributed to the
replacement session. Use `launch_id`, `app_server_pid`, and `offset` to follow
the same process across a switch.

Capture is best effort and never blocks the app-server: a single stderr record
over 1 MiB is dropped whole, the oldest queued records are shed under sustained
pressure, and shutdown waits at most one second for buffered output and one more
for the final batch. If capture fails to initialize, Omnigent logs a
`harness_diagnostic_capture_failed` warning (exception class, session ID, and
PID only) and the exception class also appears as `stderr_capture_error_type` in
the Codex startup failure snapshot (`event_name = 'codex_thread_start_failed'`).

<div className="note">

**Note:** When capture is enabled, these batched `INFO` events replace the
per-line `DEBUG` logging of Codex app-server stderr. With the flag off, that
`DEBUG` path is unchanged and no collector thread is created.
