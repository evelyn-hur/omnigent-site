...
Resetting back to the agent's default with `/model default` is only available on
harnesses that can clear their live model; native CLI wrappers such as Claude
Code and Codex accept explicit model names only. See
[Switch models mid-session](/docs/interact/web-ui#switch-models-mid-session) in
the Web UI docs for the per-harness behavior.
